...
const MonochromePortal = ({ isDemo = true, schoolName = "Elite School", logoUrl, onSearch, searchFields = ['roll_number', 'student_name'], demoResult , availableClasses, hideClassSelector }: PortalProps) => {
            ...
            <div ref={resultRef} className="relative">
              ...
              <div className="relative bg-gray-900/80 backdrop-blur-sm border sm:border-2 border-gray-700 rounded-xl sm:rounded-2xl p-3 sm:p-8 shadow-2xl">
                ...
                <div className="text-center mb-4 sm:mb-6">
                  <p className="text-lg sm:text-2xl text-white font-bold mb-1">{result.name || result.student_name}</p>
                  {result.father_name && <p className="text-sm text-gray-500 mb-3 sm:mb-4">Father: {result.father_name}</p>}
                  <div className="flex justify-center gap-2 sm:gap-4 flex-wrap">
                    <span className="px-3 sm:px-4 py-1.5 sm:py-2 bg-black/50 border border-gray-700 rounded-lg text-xs sm:text-base text-gray-300">Class: <strong className="text-white">{result.class || result.class_name}</strong></span>
                ...
                <div className="bg-black/50 border border-gray-800 rounded-lg sm:rounded-xl p-2 sm:p-6 mb-4 sm:mb-6 -mx-1 sm:mx-0"><table className="w-full text-xs sm:text-base"><thead><tr className="border-b-2 border-gray-700"><th className="text-left py-2 sm:py-3 px-1 text-gray-400 font-bold">Subject</th><th className="text-center py-2 sm:py-3 px-1 text-gray-400 font-bold w-14 sm:w-auto">Obtained</th><th className="text-center py-2 sm:py-3 px-1 text-gray-400 font-bold w-14 sm:w-auto">Total</th></tr></thead><tbody>{result.subjects?.map((subject: any, idx: number) => (<tr key={idx} className="border-b border-gray-800"><td className="py-1.5 sm:py-3 px-1 text-gray-200">{subject.subject || subject.name}</td><td className="text-center py-1.5 sm:py-3 px-1 text-white font-semibold">{subject.obtained_marks || subject.obtained}</td><td className="text-center py-1.5 sm:py-3 px-1 text-gray-500">{subject.total_marks || subject.total}</td></tr>))}<tr className="border-t-2 border-gray-700 bg-white/5"><td className="py-2 sm:py-3 px-1 text-gray-400 font-bold">TOTAL</td><td className="text-center py-2 sm:py-3 px-1 text-white font-bold">{result.total_obtained || result.subjects?.reduce((sum: number, s: any) => sum + (s.obtained_marks || s.obtained || 0), 0)}</td><td className="text-center py-2 sm:py-3 px-1 text-gray-500 font-bold">{result.total_marks || result.subjects?.reduce((sum: number, s: any) => sum + (s.total_marks || s.total || 0), 0)}</td></tr></tbody></table></div>
                <div className="grid grid-cols-3 gap-2 sm:gap-4">
                  <div className="bg-white rounded-lg sm:rounded-xl p-2 sm:p-4 text-center shadow-lg"><p className="text-gray-500 text-[10px] sm:text-xs mb-0.5">Percentage</p><p className="text-sm sm:text-2xl font-bold text-black">{typeof result.percentage === 'number' ? `${result.percentage.toFixed(2)}%` : result.percentage}</p></div>
                  ...
                  <div className="bg-black border border-gray-700 rounded-lg sm:rounded-xl p-2 sm:p-4 text-center shadow-lg"><p className="text-gray-500 text-[10px] sm:text-xs mb-0.5">Remarks</p><p className="text-[10px] sm:text-sm font-bold text-white leading-tight">{result.remarks || 'Excellent'}</p></div>
                </div>
              </div>
            </div>
            <ResultActions studentName={result.name || result.student_name} schoolName={schoolName} percentage={typeof result.percentage === 'number' ? `${result.percentage.toFixed(2)}%` : result.percentage} grade={result.grade} resultRef={resultRef} variant="monochrome" />
          </>
        )}
      </main>
    </div>
