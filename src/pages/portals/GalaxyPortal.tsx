...
const GalaxyPortal = ({ isDemo = true, schoolName = "Star Light Academy", logoUrl, onSearch, demoResult }: PortalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => { e.preventDefault(); if (!selectedClass || !studentName) { toast.error('Please select a class and enter your name'); return; } setLoading(true); setError(''); setResult(null); if (isDemo) { setTimeout(() => { setResult(generateDemoResult(studentName, selectedClass)); setLoading(false); toast.success('Result loaded successfully!'); }, 1000); } else if (onSearch) { try { const r = await onSearch(selectedClass, studentName); if (r) { setResult(r); toast.success('Result loaded successfully!'); } else { setError('No result found.'); } } catch { setError('An error occurred.'); } finally { setLoading(false); } } };
      ...
      <main className="relative z-10 container mx-auto px-3 sm:px-4 pt-14 sm:pt-6 pb-20 max-w-3xl">
        <header className="text-center mb-6 sm:mb-12">
          ...
          <div className="relative inline-block mb-3 sm:mb-6">
            <Sparkles className="absolute -top-1 sm:-top-4 -left-1 sm:-left-4 w-4 h-4 sm:w-8 sm:h-8 text-purple-400 animate-pulse" />
            <Sparkles className="absolute -top-1 sm:-top-4 -right-1 sm:-right-4 w-4 h-4 sm:w-8 sm:h-8 text-blue-400 animate-pulse" />
            <h1 className="relative text-xl sm:text-5xl md:text-6xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 via-pink-400 to-blue-400 px-6 sm:px-0">{schoolName}</h1>
          </div>
