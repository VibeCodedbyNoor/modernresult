    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-900 relative overflow-hidden">
      ...
      <main className="relative z-10 container mx-auto px-3 sm:px-4 pt-14 sm:pt-12 pb-20 max-w-4xl">
        <header className="text-center mb-6 sm:mb-12">
          ...
          <div className="relative"><h1 className="relative text-2xl sm:text-5xl md:text-6xl font-black text-white mb-2 sm:mb-3" style={{ textShadow: '0 0 20px rgba(59, 130, 246, 0.8)' }}>{schoolName}</h1></div>
        ...
        <div className="relative mb-4 sm:mb-8">
          ...
          <div className="relative bg-slate-800/80 backdrop-blur-sm border border-blue-500/30 rounded-xl sm:rounded-2xl p-4 sm:p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
              ...
              <button type="submit" disabled={loading} className="w-full py-3 sm:py-4 px-6 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-sm sm:text-base text-white font-bold rounded-lg transition-all shadow-lg disabled:opacity-50">{loading ? 'PROCESSING...' : 'INITIATE QUERY'}</button>
            </form>
          </div>
        </div>

        {error && (<div className="relative mb-4 sm:mb-8"><div className="absolute -inset-0.5 bg-red-500 rounded-xl sm:rounded-2xl opacity-50 blur"></div><div className="relative bg-slate-800/80 backdrop-blur-sm border border-red-500/30 rounded-xl sm:rounded-2xl p-4 sm:p-6"><p className="text-red-400 text-sm sm:text-base font-semibold text-center">{error}</p></div></div>)}
          ...
          <>
            <div ref={resultRef} className="relative">
              ...
              <div className="relative bg-slate-800/80 backdrop-blur-sm border border-blue-500/30 rounded-xl sm:rounded-2xl p-3 sm:p-8">
                <div className="text-center mb-4 sm:mb-6 pb-3 sm:pb-4 border-b border-blue-500/30"><h3 className="text-base sm:text-xl font-bold text-blue-400 mb-0.5 sm:mb-1">{schoolName}</h3><p className="text-blue-300/70 text-xs sm:text-sm">RESULT DATA OUTPUT</p></div>
                <div className="text-center mb-4 sm:mb-6">
                  <p className="text-lg sm:text-2xl text-white font-bold mb-1">{result.name || result.student_name}</p>
                  {result.father_name && <p className="text-sm text-blue-300/70 mb-3 sm:mb-4">Father: {result.father_name}</p>}
