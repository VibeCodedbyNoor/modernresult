...
const RetroPortal = ({ isDemo = true, schoolName = "Government High School", logoUrl, onSearch, demoResult }: PortalProps) => {
      ...
      <main className="container mx-auto px-3 sm:px-4 pt-14 sm:pt-12 pb-20 max-w-3xl">
        ...
        <div className="bg-[#e8d4b0] border-2 sm:border-4 border-[#8b6f47] rounded-lg p-4 sm:p-8 mb-4 sm:mb-8 shadow-[4px_4px_0px_#8b6f47] sm:shadow-[8px_8px_0px_#8b6f47]">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
            ...
            <div><label className="block text-xs sm:text-sm font-bold text-[#5d4a3a] mb-1.5 sm:mb-2">◆ Student Name</label><input type="text" value={studentName} onChange={(e) => setStudentName(e.target.value)} className="w-full px-3 sm:px-4 py-2.5 sm:py-3 border-2 sm:border-3 border-[#8b6f47] bg-[#f4e4c1] text-sm sm:text-base text-[#4a3728] rounded placeholder-[#8b6f47]/60 focus:outline-none focus:border-[#5d4a3a] transition-colors" placeholder="Enter your name..." required /></div>
          ...
          <>
            <div ref={resultRef} className="bg-[#e8d4b0] border-2 sm:border-4 border-[#8b6f47] rounded-lg p-3 sm:p-8 shadow-[4px_4px_0px_#8b6f47] sm:shadow-[8px_8px_0px_#8b6f47]">
              ...
              <div className="text-center mb-4 sm:mb-6">
                <p className="text-base sm:text-2xl text-[#5d4a3a] font-bold mb-3 sm:mb-4">{result.name}</p>
                <div className="flex justify-center gap-2 sm:gap-4 flex-wrap">
                  <span className="px-3 sm:px-4 py-1.5 sm:py-2 bg-[#f4e4c1] border-2 border-[#8b6f47] rounded text-xs sm:text-base text-[#4a3728] font-semibold">Class: {result.class}</span>
              ...
              <div className="bg-[#f4e4c1] border-2 sm:border-3 border-[#8b6f47] rounded p-2 sm:p-6 mb-4 sm:mb-6 -mx-1 sm:mx-0">
                <table className="w-full text-xs sm:text-base">
                  <thead><tr className="border-b-2 border-[#8b6f47]"><th className="text-left py-2 sm:py-3 px-1 text-[#4a3728] font-bold">Subject</th><th className="text-center py-2 sm:py-3 px-1 text-[#4a3728] font-bold w-14 sm:w-auto">Obtained</th><th className="text-center py-2 sm:py-3 px-1 text-[#4a3728] font-bold w-14 sm:w-auto">Total</th></tr></thead>
                  <tbody>
                    {result.subjects?.map((subject: any, idx: number) => (<tr key={idx} className="border-b border-[#8b6f47]/30"><td className="py-1.5 sm:py-2 px-1 text-[#5d4a3a] font-semibold">{subject.subject}</td><td className="text-center py-1.5 sm:py-2 px-1 text-[#4a3728] font-bold">{subject.obtained_marks}</td><td className="text-center py-1.5 sm:py-2 px-1 text-[#8b6f47]">{subject.total_marks}</td></tr>))}
                    <tr className="border-t-2 border-[#8b6f47]"><td className="py-2 sm:py-2 px-1 text-[#4a3728] font-bold">TOTAL</td><td className="text-center py-2 sm:py-2 px-1 text-[#4a3728] font-bold">{result.total_obtained}</td><td className="text-center py-2 sm:py-2 px-1 text-[#8b6f47] font-bold">{result.subjects?.reduce((sum: number, s: any) => sum + s.total_marks, 0)}</td></tr>
                  </tbody>
                </table>
              </div>
              <div className="grid grid-cols-3 gap-2 sm:gap-4">
                <div className="bg-[#d4a574] border-2 border-[#8b6f47] rounded-lg p-2 sm:p-4 text-center shadow-[2px_2px_0px_#8b6f47] sm:shadow-[4px_4px_0px_#8b6f47]"><p className="text-[#5d4a3a] text-[10px] sm:text-xs mb-0.5">★ Percentage ★</p><p className="text-sm sm:text-2xl font-bold text-[#4a3728]">{result.percentage}</p></div>
                ...
                <div className="bg-[#d4a574] border-2 border-[#8b6f47] rounded-lg p-2 sm:p-4 text-center shadow-[2px_2px_0px_#8b6f47] sm:shadow-[4px_4px_0px_#8b6f47]"><p className="text-[#5d4a3a] text-[10px] sm:text-xs mb-0.5">★ Remarks ★</p><p className="text-[10px] sm:text-sm font-bold text-[#4a3728] leading-tight">{result.remarks}</p></div>
              </div>
            </div>
            <ResultActions studentName={result.name} schoolName={schoolName} percentage={result.percentage} grade={result.grade} resultRef={resultRef} variant="retro" />
          </>
        )}
      </main>
    </div>
