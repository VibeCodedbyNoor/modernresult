...
const MinimalistPortal = ({ isDemo = true, schoolName = "Al-Huda Model School", logoUrl, onSearch, demoResult }: PortalProps) => {
      ...
      <main className="container mx-auto px-3 sm:px-4 pt-14 sm:pt-16 pb-20 max-w-2xl">
        ...
        <div className="mb-6 sm:mb-12">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
            ...
            <div className="group"><label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">Student Name</label><input type="text" value={studentName} onChange={(e) => setStudentName(e.target.value)} className="w-full px-3 sm:px-4 py-2.5 sm:py-3 border border-gray-300 bg-white text-sm sm:text-base text-black focus:outline-none focus:border-black transition-all" placeholder="Enter your name" required /></div>
        ...
        {result && !error && (
          <>
            <div ref={resultRef} className="border-t border-black pt-4 sm:pt-8 bg-white p-3 sm:p-6">
              ...
              <div className="mb-3 sm:mb-6"><p className="text-xs sm:text-sm text-gray-600 mb-0.5 sm:mb-1">Student Name</p><p className="text-base sm:text-lg font-semibold text-black">{result.name}</p></div>
